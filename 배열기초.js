...
// ┗ 파괴적 메서드 : 배열 객체의 내용을 변경하거나 추가, 삭제를 함
// ┗ 비파괴적 메서드 : 배열 객체의 내용이 변경되거나 추가, 삭제 되면 새로운 배열 객체를 생성함

// forEach() : 배열을 순회, 비파괴적 메서드
...
// pop() : 배열의 마지막 요소를 제거하고 제거된 값 반환, 파괴적 메서드
console.log(arr.pop());
console.log(arr);

// push() : 배열의 끝에 요소를 추가하고, 추가 후의 새로운 length(길이)를 반환
console.log(arr.push(100)); // 길이가 출력됨
...
// 성적이 90점 이상만 골라내기
const passPeople = people.filter(e => e.score >= 90);
console.log(passPeople);

// 도시가 서울인 배열 객체만 골라내기
const cityPeople = people.filter(e => e.city === "서울");
...
// 배열로 구성된 객체에서 도시만 추출하기
const city = people.map(person => person.city);
console.log(city);
